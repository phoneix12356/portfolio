import { FiMail, FiGithub, FiLinkedin, FiExternalLink, FiCode, FiCpu, FiLayout, FiDatabase } from "react-icons/fi";
...
const siteConfig = {
  ...
  subTitle: "Specializing in Java, Spring Boot & React",
...
const skills = [
  { name: "Backend", icon: <FiCpu />, items: ["Java", "Spring Boot", "JPA/Hibernate", "Node.js"] },
  { name: "Frontend", icon: <FiLayout />, items: ["React", "JavaScript (ES6+)", "TailwindCSS", "Framer Motion"] },
  { name: "Database & Tools", icon: <FiDatabase />, items: ["MySQL", "PostgreSQL", "MongoDB", "Git", "REST APIs"] },
];
      ...
      <header className="fixed top-0 inset-x-0 z-50 glass border-b border-white/5 backdrop-blur-md">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          >
            ...
            <span className="font-bold tracking-tight text-lg hidden sm:block">{siteConfig.name}</span>
          </motion.div>

          <nav className="flex items-center gap-4 sm:gap-6">
            <a href="#projects" className="text-sm font-medium hover:text-indigo-400 transition-colors">Projects</a>
            <a href="#skills" className="text-sm font-medium hover:text-indigo-400 transition-colors">Skills</a>
            <div className="h-4 w-px bg-white/10 mx-1" />
            <a href={siteConfig.github} target="_blank" rel="noreferrer" className="text-white/60 hover:text-white transition-colors">
              <FiGithub className="w-5 h-5" />
            </a>
            <a href={siteConfig.linkedin} target="_blank" rel="noreferrer" className="text-white/60 hover:text-white transition-colors">
      ...
      <main className="relative pt-32 pb-24 max-w-6xl mx-auto px-6">
        ...
        <section className="min-h-[70vh] flex flex-col justify-center mb-32">
          ...
          >
            ...
            <p className="max-w-2xl text-lg md:text-xl text-slate-400 mb-10 leading-relaxed italic">
              "I translate complex backend logic into elegant, high-performance web applications using Java ecosystems and modern frontend frameworks."
            </p>
        ...
        <section id="projects" className="mb-32">
          <div className="flex items-end justify-between mb-12">
            <div>
              <h2 className="text-xs font-bold text-indigo-500 uppercase tracking-widest mb-2">Portfolio</h2>
              ...
              >
                <div className="glass rounded-3xl p-8 h-full transition-all duration-500 group-hover:border-indigo-500/30 flex flex-col">
                  <div className="flex justify-between items-start mb-6">
                    <span className="text-xs font-medium text-slate-500 border border-white/5 px-3 py-1 rounded-full">{project.type}</span>
                    <a href={project.link} target="_blank" rel="noreferrer" className="text-slate-400 hover:text-indigo-400 transition-colors">
                      <FiExternalLink className="w-6 h-6" />
                    </a>
                  </div>
                  <h4 className="text-2xl font-bold mb-4 group-hover:text-indigo-300 transition-colors">{project.title}</h4>
                  <p className="text-slate-400 mb-8 flex-grow leading-relaxed">{project.description}</p>
                  <div className="flex flex-wrap gap-2">
                    {project.tech.map((t) => (
                      <span key={t} className="text-xs px-3 py-1.5 bg-white/5 rounded-lg border border-white/5 font-medium">{t}</span>
                    ))}
                  </div>
                </div>
              </motion.div>
        ...
        <section id="skills" className="mb-32">
          <div className="mb-12 text-center max-w-2xl mx-auto">
            <h2 className="text-xs font-bold text-indigo-500 uppercase tracking-widest mb-2">Expertise</h2>
            <h3 className="text-3xl md:text-4xl font-bold mb-6">Technical Arsenal</h3>
            <p className="text-slate-400 italic">"Technology is a tool; I specialize in choosing the right one for the job."</p>
          </div>
          ...
          >
            {skills.map((skill) => (
              <motion.div key={skill.name} variants={itemVariants} className="glass rounded-2xl p-8 hover:bg-white/5 transition-colors border border-white/5">
        ...
        <section className="relative glass rounded-[2.5rem] p-12 md:p-20 text-center overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-6">
              <a href={`mailto:${siteConfig.email}`} className="px-10 py-5 bg-white text-black hover:bg-slate-200 rounded-2xl font-bold transition-all w-full sm:w-auto">
                Get in touch
              </a>
              <div className="flex items-center gap-6">
                <a href={siteConfig.github} target="_blank" rel="noreferrer" className="p-3 glass rounded-full hover:text-indigo-400 transition-colors">
                  <FiGithub className="w-6 h-6" />
                </a>
                <a href={siteConfig.linkedin} target="_blank" rel="noreferrer" className="p-3 glass rounded-full hover:text-indigo-400 transition-colors">
        ...
        <footer className="mt-24 pt-8 border-t border-white/5 text-center text-sm text-slate-500">
          <div className="flex flex-col md:flex-row items-center justify-between gap-4 py-4">
            <p>© {new Date().getFullYear()} {siteConfig.name}. Designed for the web.</p>
            <div className="flex items-center gap-6">
              <span className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-green-500 animate-pulse" /> Available for work</span>
